...
2. [Ambiente do laboratório](#2-ambiente-do-laboratório)
   - [2.1. Componentes](#21-componentes)
   - [2.2. Topologia lógica](#22-topologia-lógica)
   - [2.3. Endereços utilizados no laboratório](#23-endereços-utilizados-no-laboratório)
3. [Preparação do ambiente](#3-preparação-do-ambiente)
   - [3.1. Criação de rede isolada para o laboratório](#31-criação-de-rede-isolada-para-o-laboratório)
   - [3.2. Subida do OWASP Juice Shop](#32-subida-do-owasp-juice-shop)
   - [3.3. Subida do OWASP ZAP com interface WebSwing](#33-subida-do-owasp-zap-com-interface-webswing)
4. [Comparação metodológica: OpenVAS x OWASP ZAP](#4-comparação-metodológica-openvas-x-owasp-zap)
   - [4.1. Resultado esperado do OpenVAS/Greenbone](#41-resultado-esperado-do-openvasgreenbone)
   - [4.2. Resultado esperado do OWASP ZAP](#42-resultado-esperado-do-owasp-zap)
5. [Configuração do scan no OWASP ZAP](#5-configuração-do-scan-no-owasp-zap)
   - [5.1. Quick Start — Automated Scan](#51-quick-start--automated-scan)
   - [5.2. Execução](#52-execução)
6. [Vulnerabilidade identificada: SQL Injection](#6-vulnerabilidade-identificada-sql-injection)
   - [6.1. Alerta apresentado pelo ZAP](#61-alerta-apresentado-pelo-zap)
   - [6.2. Interpretação técnica](#62-interpretação-técnica)
7. [Validação manual com curl](#7-validação-manual-com-curl)
   - [7.1. Teste inicial com aspa simples](#71-teste-inicial-com-aspa-simples)
   - [7.2. Forçando erro de sintaxe SQL](#72-forçando-erro-de-sintaxe-sql)
8. [Prova de conceito controlada: Union-Based SQL Injection](#8-prova-de-conceito-controlada-union-based-sql-injection)
   - [8.1. Enumeração de tabelas via `sqlite_master`](#81-enumeração-de-tabelas-via-sqlite_master)
   - [8.2. Analisar a saída de dados da tabela `Users`](#82-analisar-a-saída-de-dados-da-tabela-users)
   - [8.3. Validação de hash MD5 com CrackStation](#83-validação-de-hash-md5-com-crackstation)
   - [8.4. Encadeamento do impacto observado](#84-encadeamento-do-impacto-observado)
9. [Achados complementares do OWASP ZAP](#9-achados-complementares-do-owasp-zap)
10. [Lições técnicas aprendidas](#10-lições-técnicas-aprendidas)
    - [10.1. Sobre scanners de infraestrutura](#101-sobre-scanners-de-infraestrutura)
    - [10.2. Sobre scanners DAST](#102-sobre-scanners-dast)
    - [10.3. Sobre validação manual](#103-sobre-validação-manual)
11. [Recomendações de mitigação](#11-recomendações-de-mitigação)
    - [11.1. Correções na aplicação](#111-correções-na-aplicação)
    - [11.2. Correções na infraestrutura](#112-correções-na-infraestrutura)
    - [11.3. Correções operacionais](#113-correções-operacionais)
12. [Roteiro resumido para reprodução](#12-roteiro-resumido-para-reprodução)
    - [12.1. Saída real capturada do passo 7](#121-saída-real-capturada-do-passo-7)
13. [Evidências a anexar ao relatório acadêmico](#13-evidências-a-anexar-ao-relatório-acadêmico)
14. [Modelo de conclusão acadêmica](#14-modelo-de-conclusão-acadêmica)
15. [Checklist final do aluno](#15-checklist-final-do-aluno)
...
### 12.1. Saída real capturada do passo 7

Para fins de evidência acadêmica, segue a captura completa (headers + corpo JSON) obtida ao executar o comando do **passo 7** do roteiro acima. O corpo da resposta confirma a enumeração de tabelas via `sqlite_master` já resumida na tabela da seção 8.1.
...
<details>
<summary>Corpo JSON completo da resposta (clique para expandir)</summary>
